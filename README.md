...
- **객체 크기와 책임 결정**
Facade을 이용하여 서브 시스템을 어떻게 객체로 만들 수 있고 [Flyweight](<https://github.com/foryouself83/DesignPattern/tree/master/Compound>)을 이용하여 많은 객체를 효율적으로 다룰 수 있다.   
Abstact Factory, Builder를 이용하여 다른 객체를 생성하는 책임만 갖는 객체를 만들거나 Visitor, [Command](<https://github.com/foryouself83/DesignPattern/tree/master/Command>) 등을 이용하여 다른 객체에서 요청에 대한 책임을 갖도록 객체를 만들 수 있다.
- **객체 인터페이스 명세**
...
- **디자인 패턴 선택**
 - 특정 클래스로부터 객체 생성 시 클래스 이름을 명시하면 구현에 종속되므로 객체를 직접 생성하지 않는다.
